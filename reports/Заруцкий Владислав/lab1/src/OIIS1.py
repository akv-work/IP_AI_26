...
CLASSES = train_set.classes          # список из 100 названий классов
...
print(f"Train: {len(train_set)}, Test: {len(test_set)}, "
      f"классов: {NUM_CLASSES}, размер изображения: {train_set[0][0].shape}")
...
# --- Точность по классам. Для CIFAR-100 классов 100, печатаем ТОП-10 лучших и худших ---
model.eval()
...
per_class_acc = {i: (100 * class_correct[i] / class_total[i]) for i in range(NUM_CLASSES)}
sorted_acc = sorted(per_class_acc.items(), key=lambda kv: kv[1])

print("Топ-10 ЛУЧШИХ классов:")
for i, acc in sorted_acc[-10:][::-1]:
    print(f"  {CLASSES[i]:>15s}: {acc:5.2f}%")

print("Топ-10 ХУДШИХ классов:")
for i, acc in sorted_acc[:10]:
    print(f"  {CLASSES[i]:>15s}: {acc:5.2f}%")


def _topk_bars(probs, k=10, highlight=None):
    """Возвращает (индексы, вероятности в %) для топ-k классов.
    highlight — индекс класса, который надо выделить красным."""
    idx = np.argsort(probs)[::-1][:k]
    labels = [CLASSES[i] for i in idx]
    vals = probs[idx] * 100
    colors = ['red' if (highlight is not None and i == highlight) else 'steelblue' for i in idx]
...
def visualize_test_sample(index=None):
    ...
    labels, vals, colors = _topk_bars(probs, k=10, highlight=pred_label)
    axes[1].barh(labels, vals, color=colors)
    axes[1].set_xlabel("Вероятность, %")
    axes[1].set_xlim(0, max(100, vals.max() + 5))
    axes[1].invert_yaxis()
    axes[1].set_title("Топ-10 предсказаний")
